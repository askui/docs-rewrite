...
2. If the **AskUI Desktop** app is not already open, launch it from the Windows
   Start menu: press the Windows key, type `AskUI Desktop`, and press Enter
...
5. Call `list_windows` and find the window whose `process_name` is
   `AskUI.Desktop.Windows` (title contains "AskUI").
