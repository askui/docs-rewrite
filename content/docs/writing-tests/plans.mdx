...
- **`devices`**: the device the plan runs on, by profile id from
  `devices.json` (a list in the file, one entry via the app). The name is a
  snapshot for readability; the id decides.
...
longer exists, the plan's row says so and the edit dialog offers a remove
button — pick a new device and the plan runs again.
