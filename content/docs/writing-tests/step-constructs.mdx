...
colleague: whatever they would need from you to execute it — which element,
what to expect, what to do if a dialog appears, what never to touch — is
exactly what the agent needs. If a colleague could run the step from your
wording alone, so can the agent.
