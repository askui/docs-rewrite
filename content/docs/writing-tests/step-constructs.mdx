...
```md
9. Start the export. If the success message does not appear within about
...
```md
10. Open the customer record ACME-42. If an error dialog appears instead,
...
```md
11. Click **Submit** exactly once — do NOT press it again, even if the page
    seems to hang.
12. Do NOT dismiss the license dialog; the test verifies its content first.
```
...
```md
13. Log in using the `login_to_ui` procedure with the QA username and
   password secrets.
```
...
```md
14. Sign in with the QA credentials — the secrets named `QA_USERNAME` and `QA_PASSWORD`.
```
...
```md
15. Perform an HTTP GET request to `https://example.com`.
16. Scan the QR code from the screen — the decoded text contains "wikipedia".
17. Use the `sql_database` tool to run the statement `test_data`.
```
...
```md
18. On the **server**, start the message service.
19. On the **client**, send a message to the server.
```
