...
checkout on Windows Desktop"]) --> A
  subgraph CHAIN["scope of tests/ — rules.md applies to every phase"]
    direction LR
    A["1 · tests/setup.md"]
    subgraph CO["scope of tests/checkout/ — checkout/rules.md adds rules here"]
      direction LR
      B["2 · tests/checkout/setup.md"] --> C["3 · tests/checkout/pay_with_card.md"] --> D["4 · tests/checkout/pay_with_invoice.md"] --> E["5 · tests/checkout/teardown.md"]
    end
    A --> B
