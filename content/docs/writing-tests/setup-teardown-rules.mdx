...
**Try it alone:** open `setup.md` and press **Run** (or right-click it →
**Run on**). It executes exactly once, without the setups and teardowns
that frame a test, so you can debug the steps before any case depends on
them. The same works for `teardown.md`.
