...
flowchart LR
  subgraph pc["Tester PC"]
    ...
    A["«Application under test»<br/>desktop app or browser"]
    F[("«Project»<br/>tests · prompts · reports")]
    D --> A
    D --> F
  end
  D -->|"screenshots + step text"| M["«Model provider»<br/>AskUI hub or your own"]
```
...
flowchart LR
  subgraph pc["Tester PC"]
    direction TB
    D["«AskUI Desktop»<br/>author · run · review"]
    B1["«Playwright»<br/>Browser"]
    D --> B1
  end
  subgraph ci["CI pipeline"]
    direction TB
    C["«AskUI CLI»<br/>headless runs"]
    B2["«Playwright»<br/>Browser"]
    C --> B2
  end
...
Every box is one real thing you provision, and every group is optional: a
Windows or Linux VM at your cloud provider, machines in your own lab, phones
or an [iOS simulator](/docs/devices/ios-simulator) on a Mac. Browsers are the
exception, they need no machine of their own: Playwright runs on whichever
machine started the run. The tester's Desktop and the CI pipeline drive the
same targets from the same project.
