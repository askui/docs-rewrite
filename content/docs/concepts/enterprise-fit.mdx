...
- **Your own cloud tenancy**: with
  ...
  hold. Your Anthropic account and
  [Azure AI Foundry](https://learn.microsoft.com/azure/ai-foundry/) work
  directly; **AWS Bedrock** and **GCP Vertex AI** speak their own
  authentication, so they need a gateway in front that exposes an
  Anthropic- or OpenAI-compatible endpoint, which is also how most internal
  LLM gateways are already built (including
  [Entra-protected ones](/docs/extending/model-providers)). This is what
  most IT-security reviews approve.
