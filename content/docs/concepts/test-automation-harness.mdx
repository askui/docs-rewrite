...
We build it for four things, because a CUA model left to itself is
expensive, erratic, slow and hard to hold accountable at suite scale:
...
- **Reporting you can hand to someone else** — the agent writes its
  [test report](/docs/results/run-report) while it works, so every step
  carries its interpretation, expected vs. actual and a screenshot; the run
  gets a [summary](/docs/results/run-report#summary-report) the
  [Dashboard](/docs/results/dashboard) counts, and the full
  [conversation log](/docs/results#conversation-log) stays as the audit
  trail. Evidence for a defect ticket is a by-product of the run, not extra
  work — and because it's Markdown and PNGs in the project folder, CI can
  archive it and reviewers can read it without the app.
...
What the AskUI harness gives each phase, and what it enforces:
...
- **The limits and the default behavior in error cases** — what happens when
  ...
  - **A custom tool doesn't compile** → the run aborts before the first
    ...
    You can override these per suite in
    [`rules.md`](/docs/writing-tests/setup-teardown-rules#rules) — for
    example allowing a documented recovery instead of a `BROKEN`.
