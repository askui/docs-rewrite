...
- **Enable** — the *Enable In-Flight Guidance for this project* switch; off by
  default and set per project. In-Flight Guidance is **experimental**.
...
- **Reviewer model rates** — when the reviewer runs on a custom (BYOM) model, its
  provider form includes a **Model rates** section (input / output / cache-write /
  cache-read $/Mtok, with a dot as decimal separator, e.g. `1.25`) so the report can show
  the guidance cost. Hosted reviewer
  models are priced automatically.

Settings are stored in your project at `utils/inflight.json`. A custom reviewer's
endpoint, model and API key stay on your machine (like your other model
providers); only the choice to use one is in the project file. The CLI (`askui
run`) reads the same file and honors the cadence and a hosted reviewer model; a
custom reviewer is a desktop-only choice, so headless runs review with the run's
own model instead.
...
continues, and because the check runs inline each reviewer call is **time-bounded**
(90 seconds) — a slow or hung reviewer is skipped for that step, not waited on.
