...
| **AgentOS Service** | The Windows system service that runs AgentOS, `AskUI.AgentOS.Service`. Installed by the service installer and started automatically. On installations older than 26.8 it is named `AskuiCoreService`. See [AgentOS service issues](/docs/troubleshooting/agentos-service). |
| **AgentOS Updater** | The Windows service that checks for and installs new AgentOS versions, `AskUI AgentOS Updater`. Checks every 2 days and installs without administrator rights. See [Updating](/docs/agentos/installation/updating). |
