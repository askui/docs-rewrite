...
2. Pick the run in the runs sidebar on the left — it lists every run of the
   ...
   conversation.
3. Follow along: **Autofollow** keeps the view pinned to the newest entry,
   and the density toggle switches between **Quiet** (just the story),
   **Detailed** (the default), and **Raw** (the underlying messages as JSON).
