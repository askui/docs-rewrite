...
it adds a short **advisory** to your review:

- a plain-language description of what looked wrong,
- any relevant screenshots from the run to look at, and
- a pointer to email an AskUI solution engineer at **support@askui.com**.
...
- **Choose the analysis model.** We recommend — and default to — the **best
  available** model. Dreaming runs only occasionally, so stronger reasoning over
  your runs is worth far more than saving a few cents. You can instead pick "same
  as your test runs" or a specific model. (This applies on the AskUI hub; a
  bring-your-own-model setup always analyzes on your configured model.)
